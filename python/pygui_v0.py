...
var_status = tk.StringVar(name="Not Connected")
...
def Initdevices():
    ...
    dl.setSinWave(fg_handle, ch, 10, 100, 0, 0)
    dl.Scopeset(scope_handle)
    var_status = tk.StringVar(name="Connected")
...
label_status = tk.Label(ctrlframe,text = var_status, width=15)
...
label_auto =  tk.Button(ctrlframe,text = "Autoset", width=15, command = Auto)
label_dat = tk.Button(ctrlframe,text = "Graph Data", width=15, command = Graph)
label_exit = tk.Button(ctrlframe,text = "Exit", width=15, command = quit)
...
label_exit.grid(row=7,column=0,pady=(10,10))
